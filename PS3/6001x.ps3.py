...
# -*- coding: utf-8 -*-
"""
Created on Sun May 20 15:18:26 2018

@author: macintosh
Problem Set 3

We will 'scaffold' this problem, guiding you through the creation of helper functions before you implement the actual game.
...
def getAvailableLetters(lettersGuessed):
    ...
    for i in range (len(lettersGuessed)):
        if lettersGuessed[i] in answer:
            end = lettersGuessed[i]
            answer.remove (end)
    ...
    while secretWord != stringAnswer:
        ...
        if guessInLowerCase in lettersGuessed:
            ...
            if guessInLowerCase in availableLetters:
                ...
                availableLetters.remove(guessInLowerCase)
                availableLetters = ''.join(availableLetters)
            stringAnswer = getGuessedWord(secretWord, lettersGuessed)
            stringAnswer = list(stringAnswer)
            for num in stringAnswer[:]:  
                if num == " ":
                    stringAnswer.remove(num)
            stringAnswer = ''.join(stringAnswer)
