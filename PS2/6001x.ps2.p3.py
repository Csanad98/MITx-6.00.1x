...
# -*- coding: utf-8 -*-
"""
Created on Sun May 20 15:00:05 2018
...
Well then, how can we calculate a more accurate fixed monthly payment than
 ...
 make this program run faster using a technique introduced in lecture - bisection 
 search!
...
such that we can pay off the entire balance within a year. What is a reasonable
 lower bound for this payment value? $0 is the obvious anwer, but you can do better 
 than that. If there was no interest, the debt can be paid off by monthly payments
 of one-twelfth of the original balance, so we must pay at least this much every
...
lowbound = balance/12
upbound = (balance * ((1 + annualInterestRate/12)** 12 ))/12

while round(lowbound, 2) != round (upbound, 2):

    lowpay = (upbound + lowbound)/2
    zero = balance
    for i in range (12):
        zero = zero - lowpay
        zero = zero + (zero *(annualInterestRate/12))
