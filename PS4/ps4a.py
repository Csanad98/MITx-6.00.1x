...
def getWordScore(word, n):
    ...
    for i in range (len(word)): # iterate through each letter of the word
        if word[i] in SCRABBLE_LETTER_VALUES: # if the i-th letter of the word is in the dictionary of letters and values
            score += SCRABBLE_LETTER_VALUES.get(word[i], 0) # add the value allocated to the given letter to score
    score = score*len(word)   #calculate the score by multiplying the sum of letter values by the length of the word 
    
    if n == len(word):
        score += 50 # add the extra 50 points to the score if all letters have been used in the word
